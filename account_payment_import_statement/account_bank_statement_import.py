...
#        'currency_id': fields.related('journal_id', 'currency', type='many2one', relation='res.currency', string='Currency', readonly=True),
    ...
    def create(self, cr, uid, vals, context=None):
        wizard_id = super(account_bank_statement_import_wizard, self).create(cr, uid, vals, context=context)
...
#        if existing_st_ids:
    ...
    def _wizard_fill_next_statement(self, cr, uid, wizard_id, context=None):
        ...
        try:
            self._wizard_fill_from_statement(cr, uid, wizard_id, wizard_state['statements'][current_statement], context=context)
        except StatementImportError, e:
            self._wizard_set_error(cr, uid, wizard_id, e.args[0], context=context)
            return False
    ...
    def _get_journal_from_statement(self, cr, uid, statement, context=None):
        ...
        if account_type == 'standard':
            ...
            raise Exception('Unknown bank account type of type = %s' % (account_type))

        partner_bank_obj = self.pool.get('res.partner.bank')
        payment_mode_obj = self.pool.get('payment.mode')

        partner_bank_domain = ['|',(account_field,'=',account_number.lower()),(account_field,'=',account_number.upper())]
        partner_bank_ids = partner_bank_obj.search(cr, uid, partner_bank_domain, context=context)
        if not partner_bank_ids:
            raise StatementImportError('No IBAN found for: %s [type: %s]' % (account_number, account_type))

        payment_mode_ids = payment_mode_obj.search(cr, uid, [('bank_id','=',partner_bank_ids[0])], context=context)
        if not payment_mode_ids:
            raise StatementImportError('No Payment Mode found for: %s [type: %s]' % (account_number, account_type))
        payment_mode = payment_mode_obj.browse(cr, uid, payment_mode_ids[0], context=context)
...
#        values['journal_id'] = st_payment_mode['journal'][0]
    ...
    def _get_currency_from_statement(self, cr, uid, statement, context=None):
        ...
        currency_ids = currency_obj.search(cr, uid, ['|',('code','=',statement_currency),('name','=',statement_currency)], context=context)
        if not currency_ids:
            raise StatementImportError('No Currency found for currency %s' % (statement_currency))
    ...
    def _get_period_from_statement(self, cr, uid, statement, context=None):
        ...
        period_ids = period_obj.search(cr, uid, [('date_start','<=',start),('date_stop','>=',end),('special','=',False)], context=context)
        if not period_ids:
            raise StatementImportError('No period found between %s and %s' % (start, end))
...
#        values['date'] = str(values['date'])
    ...
    def button_create_real_statement(self, cr, uid, ids, context=None):
        ...
        statement_currency_id = wizard.currency_id.id

        wizard_stline_pool = self.pool.get('account.bank.statement.import.wizard.line')
        ...
        for seq, line in enumerate(wizard.line_ids):
            ...
            if line.move_line_ids:
                ...
                voucher_context['date'] = line_data['date'] # use payment date for currency computation
